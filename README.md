...
**실제 측정 결과** (2026-02-10, 14,640,946 포인트 클라우드 기준)

### 📊 테스트 환경
- **포인트 클라우드**: 14,640,946 points
- **테스트 기간**: 30초
- **평면 업데이트**: 0.5초 간격 (59개 평면 발행)
- **모드**: Baseline (기본값)
...
**주요 발견**:
1. **프레임 레이트**: 약 2.5 FPS - 실시간 응용에 충분
2. **처리량**: ~3천6백만 포인트/초 - 우수한 성능
3. **버퍼링**: 132%의 응답률은 일부 평면이 빠르게 처리됨을 의미
4. **안정성**: 연속 운영 중 안정적인 성능 유지

**성능 개선 방법**:
- `enable_downsample: true` 활성화 → 최대 10배 속도 향상
- `raster_mode: "fast_stable"` 사용 → O(N log N)으로 개선
- `voxel_leaf_size` 감소 → 더 많은 다운샘플링
- `publish_rate_hz` 감소 → CPU 부하 감소
